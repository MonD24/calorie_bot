...
async def handle_photo_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        error_msg = str(e)
        logging.error(f"Error processing photo: {error_type} - {error_msg}")
        
        # Определяем тип ошибки для более дружелюбного сообщения
        ...
        if is_timeout:
            await update.message.reply_text(
                ...
                '💡 Попробуйте:\n'
                '• Отправить фото еще раз (бот автоматически сделает несколько попыток)\n'
                '• Описать блюдо текстом для расчета калорий\n'
                '• Подождать пару минут и повторить'
            )
        elif is_overloaded:
            await update.message.reply_text(
                '⚠️ Сервер перегружен. Слишком много запросов.\n\n'
                '💡 Попробуйте:\n'
                '• Подождать 1-2 минуты и отправить фото снова\n'
                '• Описать блюдо текстом для расчета калорий'
            )
        else:
            await update.message.reply_text(
                ...
                f'💡 Попробуйте:\n'
                f'• Отправить фото еще раз\n'
                f'• Описать блюдо текстом для расчета калорий\n'
                f'• Обратиться к разработчику, если ошибка повторяется'
            )
